---
name: explanation
description: Load when the user asks to be quizzed, drilled, tested, or examined on code they built, or is preparing for a lesson defense, demo walkthrough, or checkpoint review. Do not use for an ordinary "explain this code" request; this skill examines the user and never answers its own questions.
metadata:
  upstream: none
---
...
instructor does - which parts of this system the user can actually explain.
...
1. Scope the drill.
   ...
   - Read `AGENTS.md` and the current lesson `README.md`, then read the
     implementation completely before asking the first question.
   - Confirm how much time the user has. A standard drill is 6-10 questions.

2. Build the question set from their actual code.
   - Every question is anchored to a real file and line in the user's own
...
3. Conduct the drill.
   ...
   - The user may say "skip"; record it as missing and move on.
...
4. Hold the line while the drill is active.
   ...
   - If the user asks you to "just tell me," offer to end the drill instead.
...
5. Deliver the scorecard.
   ...
   - Record only what was observed: the user could not explain the referenced
     code. Do not infer who wrote it or how it was produced.
   - Suggest the user record the scorecard in today's journal themselves.
...
- Do not write the journal entry or the scorecard reflection for the user.
- After the drill ends, normal rules apply again (including `api-docs-first`);
  answers to drill questions still come from the user's own review, not
  from you.

## Done When

- [ ] The implementation was read in full before the first question.
- [ ] Every question was anchored to the user's actual code.
